    ...
    if request.method =="POST":
        ...
        if user is not None:
            try:
                ...
                is_cart_item_exists = CartItem.objects.filter(cart=cart).exixts()
                if is_cart_item_exists:
                    ...
                    for pr in product_variation:
                        if pr in ex_var_list:
                            ...
                            item_id = id[index]
                            item = CartItem.ojects.get(id=item_id)
                            item.quantity += 1
                            item.user = user
                            item.save()
                        else:
...
def forgot_password(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string('shop/accounts/forgot_password/send_resetpassword_email.html', {
                'user':user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),
                'token': account_activation_token.make_token(user),
            ...
            return redirect('/account/forgot_password/?command=resetpassword&email='+email)
        else:
            messages.error(request, 'This email does not exist')
            return redirect('accounts:forgot_password')
        
    return render(request, 'shop/accounts/forgot_password/forgot_password.html')
...
def reset_password(request):
    if request.method == 'POST':
        ...
        return render(request, 'shop/accounts/forgot_password/reset_password.html')
