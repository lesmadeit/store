...
def add_cart(request, product_id):
    ...
    if current_user.is_authenticated:
        ...
        if referer:
            ...
            if reverse('shop:shop') in url_parts.path:
                # If the referer is the shop page, preserve the pagination query parameter

                redirect_url = reverse('shop:shop')
                if 'page' in query_params:
                    redirect_url = f"{redirect_url}?{urlencode({'page': query_params['page'][0]})}"
